...
def handle_client(client, addr, server, roll):
    connected = True
    while connected:
        ...
        if message_from_client.lower() == "terminate":
            ...
            if roll == "publisher":
                for i in clients:
                    if i[1] == "subscriber":
                        i[0].send(f"[From publisher  :  {addr} :- {message_from_client}]".encode())
...
def start_server(port):
    ...
    while True:
        ...
        client_roll = client_socket.recv(1024).decode()
        clients.append((client_socket, client_roll))
        print(f"[Client - {client_addr} is Listening as a {client_roll}]")
        client_handler = threading.Thread(target=handle_client, args=(client_socket, client_addr, server_socket, client_roll))
